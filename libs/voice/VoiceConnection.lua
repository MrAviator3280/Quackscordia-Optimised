...
local function check(n, mn, mx)
	if not tonumber(n) or n < mn or n > mx then return error(format('Value must be a number between %s and %s', mn, mx), 2) end
	return n
end
...
function VoiceConnection:getBitrate() return self._encoder:get(opus.GET_BITRATE_REQUEST) end
...
function VoiceConnection:getComplexity() return self._encoder:get(opus.GET_COMPLEXITY_REQUEST) end
...
function VoiceConnection:_parseAudioPacket(packet, key)
	...
	if #packet < 12 then return nil, 'packet too short' end
	...
	if payload_len < 0 then return nil, 'invalid payload length' end
	...
	if has_padding then
		...
		if padding_len > message_len then return nil, 'invalid padding length' end
...
function VoiceConnection:playPCM(source, duration)
	...
	if not self._ready then return nil, 'Connection is not ready' end
...
function get.channel(self) return self._channel end

function get.client(self) return self._client end
